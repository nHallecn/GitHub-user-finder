...
const joinedDateElement = document.getElementById("joined-date"); // Note: HTML ID was missing, now fixed in index.html
...
async function searchUser(){
    ...
    if(!username) {
        errorContainer.textContent = "Please enter a username to search.";
        showError();
        return;
    }
...
async function fetchRepositories(reposUrl){
    ...
    try{
        // Added sort and direction to fetch latest public repos
        const response = await fetch(reposUrl + "?per_page=10&sort=pushed&direction=desc");
...
function displayRepos(repos){
  ...
  <div class="repo-meta">
    ...
    <div class="repo-meta-item">
      <i class="fas fa-history"></i> Updated ${updatedAt}
    </div>
  </div>
`; 
...
function displayUserData(user){
    ...
    // Set the text content for the joined date span
    ...
    // Company: Show/hide container if null (best practice)
    if(user.company){
        companyElement.textContent = user.company;
        companyContainer.style.display = "flex";
    } else {
        companyElement.textContent = "Not specified";
        // companyContainer.style.display = "none"; // Option to hide container if not specified
    }

    // Blog: Link logic and text content
    if(user.blog){
